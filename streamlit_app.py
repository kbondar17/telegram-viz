...
st.markdown('''В начале 2021 года ежемесячная аудитория Telegram превысила 500 млн пользователей. Помимо высокого уровня приватности юзеров привлекает контроль над своим медиапотреблением – телеграм-каналы позволяют обойти рекомендательные алгоритмы и цензуру соцсетей. Однако разобраться в обилии авторов может быть непросто. Перед вами попытка сделать краткий гид по общественно-политическим и новостным каналам.''')
st.markdown('''Граф ниже иллюстрирует связи между каналами из нашей выборки. Чем ближе они находятся друг к другу, тем больше взаимных упоминаний и репостов они делают. Размер узлов соответствует количеству подписчиков. Цветом выделены кластеры, представители которых связаны между собой сильнее, чем с другими.''')
...
st.markdown('''Кажется, взаимоотношения в телеграме хорошо отражают политические реалии. Любопытны "подкластеры" - **Алексей Навальный** и **Михаил Ходорковский**, окруженные созданными ими же информационными ресурсами.
...
В поле ниже можно выбрать конкретный канал и посмотреть, какие темы он освещает. Например, **Baza** пишет почти исключительно про криминал и происшествия,
 **"Открытые медиа"** следят за протестами, а **"Незыгарь"** одержим перестановками во власти.
'''
)
...
st.markdown('Теперь понаблюдаем, как распределение тем меняется во времени в зависимости от типа источника. Для простоты визуализации разделим источники на две группы – "провластные" и "независимые". /'
            'В этом процессе опираемся на собственный анализ контента, а также на расследования [СМИ]("https://www.proekt.media/narrative/telegram-kanaly/")')

with st.expander('посмотреть какой канал в какой группе'):
...
st.markdown('Любопытно, что, например, обострение на востоке Украины в конце 2020 и начале 2021 или ситуация в США явно сильнее интересовали прокремлевские СМИ. Обратная ситуация с протестами в Белоруссии или судебными процессами в РФ – публичные независимые авторы комментировали эту тему гораздо охотнее.')

st.markdown('Подобные визуализации информационных потоков помогают подобрать источники из разных кластеров /'
            'и избежать попадания в "эхо-камеру". <br> /'
            'В работе использована библиотека Gensim и алгоритм моделирования LDA. Подробнее по [ссылке](https://webdevblog.ru/tematicheskoe-modelirovanie-s-pomoshhju-gensim-python)' )